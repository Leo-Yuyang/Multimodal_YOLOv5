...
from .triplet_attention_gy import *
from .fca_layer import FcaLayer as FCA
...
class Detect(nn.Module):
    ...
    def _make_grid(nx=20, ny=20):
        ...
        return torch.stack((xv, yv), 2).view((1, 1, ny, nx, 2)).float()#size:[1,1,20,20,2]
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_planes, in_planes // 16, 1, bias=False).half(),
                                nn.ReLU().half(),
                                nn.Conv2d(in_planes // 16, in_planes, 1, bias=False).half())#转换为半浮点数
    ...
    def forward(self, x):
        ...
        out = avg_out + max_out
        return self.sigmoid(out)#shape:B,inplanes,1,1
...
class Model(nn.Module):
    ...
    def eca_layer(self, x, gamma=2, b=1):
        ...
        N, C, H, W = x.size()
        t = int(abs((math.log(C, 2) + b) / gamma))#t=(log2^c +b)/gamma
        k_size = t if t % 2 else t + 1
        # k_size = 3
    ...
    def forward(self, x, augment=False, profile=False,Tag=False):
        ...
        if augment:
            ...
            for si, fi in zip(s, f):
                xi = scale_img(x.flip(fi) if fi else x, si, gs=int(self.stride.max()))#gs=32
                print(xi.shape)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv,Conv_t, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus,Focus_t, CrossConv, BottleneckCSP,
                 C3,C3_t,C4,multi_mix_t,SPP_t,C3_t_cat,multi_mix_CBL_t,multi_mix_CBL_PLARD_t,multi_mix_CBL_PLARD_modify_t,multi_mix_CBL_PLARD_modified_deformal_t,multi_mix_CBL_PLARD_modified_wh_t]:
            c1, c2 = ch[f], args[0]
